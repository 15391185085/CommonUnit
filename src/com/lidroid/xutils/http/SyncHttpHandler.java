...
public class SyncHttpHandler {
	...
	private String _getMethodRequestUrl; // If current request not http "get"
		...
		while (retry) {
			...
			try {
				...
				if (_getMethodRequestUrl != null) {
					String result = HttpUtils.sHttpGetCache
							.get(_getMethodRequestUrl);
					if (result != null) {
	...
	private ResponseStream handleResponse(HttpResponse response)
		...
		if (statusCode < 300) {
			...
			return new ResponseStream(response, charset, _getMethodRequestUrl,
					expiry);
