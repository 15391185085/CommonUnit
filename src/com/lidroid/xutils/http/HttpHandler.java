...
public class HttpHandler<T> extends CompatibleAsyncTask<Object, Object, Object>
		...
		while (retry) {
			...
			try {
				if (request.getMethod().equals(
						HttpRequest.HttpMethod.GET.toString())) {
	...
	protected void onProgressUpdate(Object... values) {
		...
		case UPDATE_FAILURE:
			this.stop();
			if (callback != null) {
				callback.onFailure((HttpException) values[1],
						(String) values[2]);
			}
			break;
		case UPDATE_SUCCESS:
	...
	private Object handleResponse(HttpResponse response) throws HttpException,
			IOException {
		if (response == null) {
		...
		if (statusCode < 300) {
			...
			if (entity != null) {
				...
				if (isDownloadingFile) {
					...
					responseBody = mFileDownloadHandler.handleEntity(entity,
							this, fileSavePath, autoResume, responseFileName);
				} else {

					// Set charset from response header info if it's exist.
					String responseCharset = OtherUtils
							.getCharsetFromHttpResponse(response);
					charset = TextUtils.isEmpty(responseCharset) ? charset
							: responseCharset;

					responseBody = mStringDownloadHandler.handleEntity(entity,
							this, charset);
					HttpUtils.sHttpGetCache.put(_getRequestUrl,
							(String) responseBody, expiry);
				}
			}
			return responseBody;
